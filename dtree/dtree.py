...
class DT:
    def __init__(self, H=8, n_features=5, stop=3):
        self.MAX_HIGH = 23
        ...
        if H is None: 
            self.cutting = False
            self.H = self.MAX_HIGH
        else:
            self.H = min(self.MAX_HIGH, H + 1)
        self.vertices = 0
    ...
    def create_node(self, v=1, depth=1):
        ...
        if (depth + 1 <= self.H or not self.cutting) and depth + 1 <= self.MAX_HIGH - 1 \
            ...
            if len(best_split) == 0: self.update_leaf(v)
            else:
    ...
    def print(self, v=1, depth=1):
        inds = [v + 1 for v in range(self.vertices) if len(self.tree[v + 1]) > 0]
        for v in inds:
            if self.split_feature[v] != []:
                print(self.split_feature[v])
                print('Q', self.split_feature[v] + 1, self.split_value[v], 2 * v, 2 * v + 1)
            else:
                print('C', self.prediction[v])
